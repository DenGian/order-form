    ...
    */ ?>
    <form method="post">
        ...
        <fieldset>
            ...
            <h6>Served with 3 eggs, Hashbrowns, Toast & Jelly</h6>
            <?php foreach ($breakfast as $i => $breakfast): ?>
                <label>
					<?php // <?= is equal to <?php echo ?>
                    <input type="checkbox" value="1" name="products[<?php echo $i ?>]"/> <?php echo $breakfast['name'] ?> -
                    &euro; <?= number_format($breakfast['price'], 2) ?></label><br />
            <?php endforeach; ?>
        </fieldset>
        <fieldset>
            <legend>Burgers</legend>
            <h6>1000 Island Dressing, Onions, Lettuce, Tomato & Pickles</h6>
            <?php foreach ($burgers as $i => $burgers ): ?>
                <label>
                    <?php // <?= is equal to <?php echo ?>
                    <input type="checkbox" value="1" name="products[<?php echo $i + 4 ?>]"/> <?php echo $burgers['name'] ?> -
                    &euro; <?= number_format($burgers['price'], 2) ?></label><br />
            <?php endforeach; ?>
        </fieldset>
        <fieldset>
            <legend>Sandwiches</legend>
            <h6>Breakfast Sandwiches Served All Day with Coffee or Soda</h6>
            <?php foreach ($sandwiches as $i => $sandwiches): ?>
                <label>
                    <?php // <?= is equal to <?php echo ?>
                    <input type="checkbox" value="1" name="products[<?php echo $i + 9?>]"/> <?php echo $sandwiches['name'] ?> -
                    &euro; <?= number_format($sandwiches['price'], 2) ?></label><br />
            <?php endforeach; ?>
        </fieldset>
